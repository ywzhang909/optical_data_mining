...
img_process_func = Callable[["SpotImage", dict[str, Any] | None], tuple["SpotImage", dict[str, Any] | None]]
...
class FunctionRegistry:
    ...
    @classmethod
    def get(cls, name: str) -> img_process_func | None:
...
def process_time_columns(df: pd.DataFrame):
    ...
    df["time"] = pd.to_datetime(
        df["time"].str.split("(").str[0].str.replace("：", ":", regex=False), format="%Y%m%d %H:%M:%S.%f"
    )
    return df
...
class Image2D:
    def __init__(self, image_array):
        self.image_array = (
            ...
            else np.array(image_array, dtype=np.float32)
        )
...
class SpotImage(Image2D):
    def __init__(self, image_array, name: str, meta_info: dict[str, Any] | None = None):
