...
def process_time_columns(df):
    ...
    # 添加time列
    df = df.with_columns([
        pl.col('path').map_elements(lambda x: x.stem).alias('time')
    ])
    # 提取括号内信息
    df = df.with_columns([
        pl.col('time').str.extract(r'[（\(]([^）\)]*)[）\)]', 1).alias('info')
    ])
    # 转换时间格式
    df = df.with_columns([
        pl.col('time').str.split('(').arr.first()
         .str.replace('：', ':', literal=True)
         .str.strptime(pl.Datetime, format='%Y%m%d %H:%M:%S.%f').alias('time')
    ])
    return df
...
power_data = pl.read_csv(str(power_file), separator='\t', encoding='gbk', has_header=False)
...
if ref_axis_img_path := list(ref_axis_dir.glob('*.TIFF')):
    ref_axis = pl.DataFrame([{'path': path} for path in ref_axis_img_path])
...
axis_beam = pl.DataFrame([{'path': path} for path in axis_beam_img_path])
pupil_beam = pl.DataFrame([{'path': path} for path in pupil_beam_img_path])

if ref_to_weak_light:
    ref_axis = ref_axis.with_columns([
        pl.col('path').map_elements(read_tiff_to_numpy, return_dtype=pl.Object).alias('img_array')
    ])
axis_beam = axis_beam.with_columns([
    pl.col('path').map_elements(read_tiff_to_numpy, return_dtype=pl.Object).alias('img_array')
])
pupil_beam = pupil_beam.with_columns([
    pl.col('path').map_elements(read_tiff_to_numpy, return_dtype=pl.Object).alias('img_array')
])
...
def process_image_data(df, denoise_method='median'):
    ...
    Args:
        df (pl.DataFrame): 包含img_array列的DataFrame
    ...
    if denoise_method == 'median':
        df = df.with_columns([
            pl.col('img_array').map_batches(lambda x: np.median(x), return_dtype=pl.Float64).alias('black')
        ])
    elif denoise_method == 'min':
        df = df.with_columns([
            pl.col('img_array').map_batches(lambda x: np.min(x), return_dtype=pl.Float64).alias('black')
        ])
    
    # 去噪后的图像
    black_threshold = df.select(pl.col('black').max()).item()
    df = df.with_columns([
        pl.col('img_array').map_batches(lambda x: np.where(x > black_threshold, x - black_threshold, 0), return_dtype=pl.Object).alias('denoise_img_array')
    ])
    
    # 计算总强度
    df = df.with_columns([
        pl.col('denoise_img_array').map_batches(lambda x: np.sum(x), return_dtype=pl.Float64).alias('intensity')
    ])
...
if ref_to_weak_light:
    ...
    valid_ref_axis = ref_axis.filter(pl.col('intensity') > 10)
valid_axis_beam = axis_beam.filter(pl.col('intensity') > 10)
valid_pupil_beam = pupil_beam.filter(pl.col('intensity') > 10)
# 提取一阶矩、二阶矩
...
def d4sigma_feature_extract(df: pl.DataFrame):
    d4sigma_features = df.select([
        pl.col('denoise_img_array').map_batches(lambda x: d4sigma(x), return_dtype=pl.Object).alias('d4sigma_data')
    ]).unnest('d4sigma_data')
    
    d4sigma_features = d4sigma_features.with_columns([
        (pl.col('D_x') * pl.col('D_y')).sqrt().alias('avg_sigma2')
    ])
    
    return df.hstack(d4sigma_features)
...
if ref_to_weak_light:
    ...
    ref_axis_center_x = valid_ref_axis.select(pl.col('center_x').mean()).item()
    ref_axis_center_y = valid_ref_axis.select(pl.col('center_y').mean()).item()
    valid_axis_beam = valid_axis_beam.with_columns([
        (pl.col('center_x') - ref_axis_center_x).alias('center_offset_x'),
        (pl.col('center_y') - ref_axis_center_y).alias('center_offset_y'),
        (((pl.col('center_x') - ref_axis_center_x) ** 2 + (pl.col('center_y') - ref_axis_center_y) ** 2).sqrt()).alias('center_offset')
    ])
...
center_axis_beam = valid_axis_beam.select([
    pl.struct(['denoise_img_array', 'center_x', 'center_y']).map_batches(lambda x: shift_to_center_fft(x['denoise_img_array'], x['center_x'], x['center_y']), return_dtype=pl.Object).alias('shifted_data')
]).unnest('shifted_data')

valid_axis_beam = valid_axis_beam.hstack(center_axis_beam)

center_pupil_beam = valid_pupil_beam.select([
    pl.struct(['denoise_img_array', 'center_x', 'center_y']).map_batches(lambda x: shift_to_center_fft(x['denoise_img_array'], x['center_x'], x['center_y']), return_dtype=pl.Object).alias('shifted_data')
]).unnest('shifted_data')

valid_pupil_beam = valid_pupil_beam.hstack(center_pupil_beam)
...
uniformity_features = valid_pupil_beam.select([
    pl.struct(['denoise_img_array', 'center_x', 'center_y']).map_batches(lambda x: uniformity(x['denoise_img_array'], (x['center_x'], x['center_y'])), return_dtype=pl.Object).alias('uniformity_data')
]).unnest('uniformity_data')

valid_pupil_beam = valid_pupil_beam.hstack(uniformity_features)

xy_profile = valid_pupil_beam.select([
    pl.struct(['denoise_img_array', 'center_x', 'center_y']).map_batches(lambda x: get_profiles(x['denoise_img_array'], (x['center_x'], x['center_y'])), return_dtype=pl.Object).alias('profile_data')
]).unnest('profile_data')
...
fit_features = xy_profile.select([
    pl.struct(['vertical']).map_batches(lambda x: calc_lr_diff(x['vertical']), return_dtype=pl.Object).alias('fit_data')
]).unnest('fit_data')

valid_pupil_beam = valid_pupil_beam.hstack(fit_features)
...
gaussian_dia = valid_axis_beam.select([
    pl.struct(['denoise_img_array', 'center_x', 'center_y']).map_batches(lambda x: calculate_xy_diameters(x['denoise_img_array'], x['center_x'], x['center_y']), return_dtype=pl.Object).alias('gaussian_data')
]).unnest('gaussian_data')

valid_axis_beam = valid_axis_beam.hstack(gaussian_dia)
...
def shape_feature_extract(df: pl.DataFrame, use_gpu=False):
    """
    提取形状特征，支持GPU加速
    ...
    """
    # 转换图像格式
    uint8_images = df.select([
        pl.col('denoise_img_array').map(convert_to_cv, return_dtype=pl.Object).alias('uint8_img')
    ])
    
    ellipse_features = uint8_images.select([
        pl.col('uint8_img').map_elements(ellipse_fit, return_dtype=pl.Object).alias('ellipse_data')
    ]).unnest('ellipse_data')
    
    circle_features = uint8_images.select([
        pl.col('uint8_img').map_elements(find_spot_border, return_dtype=pl.Object).alias('circle_data')
    ]).unnest('circle_data')
    
    return df.hstack(ellipse_features).hstack(circle_features)
...
radius_feature = valid_pupil_beam.select([
    pl.struct(['denoise_img_array', 'center_x', 'center_y']).map_batches(lambda x: calc_radius(x), return_dtype=pl.Object).alias('radius_data')
]).unnest('radius_data')

valid_pupil_beam = valid_pupil_beam.hstack(radius_feature)
...
# %%
# 光瞳光轴对齐
valid_axis_beam = valid_axis_beam.sort('time')
valid_pupil_beam = valid_pupil_beam.sort('time')

# 合并数据，按时间nearest join
if len(valid_axis_beam) < len(valid_pupil_beam):
    merged_beam = valid_axis_beam.join_asof(
        valid_pupil_beam,
        on='time',
        strategy='nearest',
        suffix='_pupil'
    ...
    merged_beam = valid_pupil_beam.join_asof(
        valid_axis_beam,
        on='time',
        strategy='nearest',
        suffix='_axis'
...
strehl_results_with_scaler = merged_beam.select([
    pl.struct(['shifted_img_array_pupil', 'shifted_img_array_axis']).map_batches(lambda x: calculate_strehl_ratio_with_energy_conservation(x['shifted_img_array_pupil'], x['shifted_img_array_axis']), return_dtype=pl.Float64).alias('strehl_data')
]).unnest('strehl_data')

merged_beam = merged_beam.hstack(strehl_results_with_scaler)
...
bpp_results = merged_beam.select([
    pl.struct(['avg_sigma2_pupil', 'avg_sigma2_axis']).map(lambda x: calculate_bpp_from_pupil_and_focal(x['avg_sigma2_pupil'], x['avg_sigma2_axis']), return_dtype=pl.Object).alias('bpp_data')
]).unnest('bpp_data')

merged_beam = merged_beam.hstack(bpp_results)
...
cols = [col for col in merged_beam.columns if merged_beam.schema[col] not in [pl.Object, pl.Utf8, pl.List, pl.Struct]]

saved_beam = merged_beam.select(cols)
saved_beam.write_parquet(str(root_dir / 'merged_beam_analysis.parquet'), compression='zstd')