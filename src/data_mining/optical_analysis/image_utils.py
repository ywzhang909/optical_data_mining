...
def load_image(file_path: Union[str, Path], as_grayscale: bool = True) -> np.ndarray:
    """
    从文件路径加载图像
...
def find_spot_border(image: np.ndarray) -> dict:
    """
    ...
    Returns:
        dict: 包含 'border_x', 'border_y', 'border_radius' 的字典
    """
    if not isinstance(image, np.ndarray):
        raise TypeError(f"find_spot_border expects numpy array, got {type(image)}")

    image_gray = np.asarray(image, dtype=np.float64)
    denoised_image = cv2.GaussianBlur(image_gray, (3, 3), 0)
