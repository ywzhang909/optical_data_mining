...
def test_visibility_device_data():
    """测试仿真选择-能见度-读取设备数据接口"""
    response = client.post(
        "/simulation/visibility/device-data",
        json={
    ...
    assert all("distance" in item and "value" in item for item in data["data"])

def test_visibility_mean():
    """测试仿真选择-能见度-均值接口"""
    response = client.post(
        "/simulation/visibility/mean",
        json={
            "start_date": "2025-07-22",
            "end_date": "2025-07-23"
        }
