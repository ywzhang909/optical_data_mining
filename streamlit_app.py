...
# ── CJK font ─────────────────────────────────────────────
# Download Noto Sans CJK SC once per session (avoids embedding ~16 MB OTF).
# Falls back to system Droid Sans Fallback if download fails.
_CJK_URL = (
    "https://cdn.jsdelivr.net/gh/notofonts/noto-cjk/"
    "Sans/OTF/SimplifiedChinese/NotoSansCJKsc-Regular.otf"
)
_CJK_PATH = os.path.expanduser("~/.fonts/NotoSansCJKsc-Regular.otf")
_has_cjk = False
if not os.path.exists(_CJK_PATH):
    os.makedirs(os.path.dirname(_CJK_PATH), exist_ok=True)
    try:
        import urllib.request
        urllib.request.urlretrieve(_CJK_URL, _CJK_PATH)
    except Exception:
        _CJK_PATH = None
if _CJK_PATH and os.path.exists(_CJK_PATH):
    try:
...
if not _has_cjk:
    ...
    if os.path.exists(_fallback):
        fm.fontManager.addfont(_fallback)

_sans = (
    ["Noto Sans CJK SC", "Droid Sans Fallback", "DejaVu Sans"]
    if _has_cjk
    else ["Droid Sans Fallback", "DejaVu Sans"]
)
...
def _render_pupil_type_analysis(
    ...
    if pupil_type == "高斯光 (Gaussian)":
        ...
        with col_x:
            st.metric("X截面 RMS", f"{uniformity.get('rms_x', np.nan):.4f}",
                      help="过圆心的水平截面内 RMS 非均匀度 = std / mean。")
        with col_y:
            st.metric("Y截面 RMS", f"{uniformity.get('rms_y', np.nan):.4f}",
                      help="过圆心的垂直截面内 RMS 非均匀度 = std / mean。")
...
def _render_sidebar():
    """Render sidebar controls and return configuration dictionary."""
    # 侧边栏 - 参数设置
    ...
    max_data_value = st.sidebar.number_input("像素最大值对应数值", value=255.0, step=0.1, format="%.4f", help="图像像素值对应的物理最大数值，用于线性缩放。")
    ...
    axis_pixel = axis_pixel * 1e-6
    pupil_pixel = pupil_pixel * 1e-6
    return dict(axis_pixel=axis_pixel, pupil_pixel=pupil_pixel,
        denoise_method=denoise_method, manual_threshold=manual_threshold,
        pupil_type=pupil_type, uniformity_boundary_type=uniformity_boundary_type,
        manual_center_x=manual_center_x, manual_center_y=manual_center_y,
        manual_radius=manual_radius, ftl_n_angles=ftl_n_angles,
        _unit_factor=_unit_factor, _unit_label=_unit_label,
        wavelength=wavelength, focal_length=focal_length,
        aperture_diameter=aperture_diameter, max_zernike_order=max_zernike_order,
...
def _display_3d_surface(has_axis, axis_denoise, has_pupil, pupil_denoise):
    """Display 3D intensity surface plots."""
...
def _display_pupil_analysis(has_pupil, has_axis, axis_features, pupil_denoise, pupil_img,
                         pupil_pixel, manual_center_x, manual_center_y,
                         manual_radius, pupil_type, ftl_n_angles,
                         _unit_factor, _unit_label, uniformity_boundary_type,
                         wavelength, focal_length):
    """Display all pupil analysis. Returns dict with results."""
    if has_pupil:
...
def _display_axis_analysis(has_axis, axis_img, axis_denoise, axis_features,
                          axis_pixel, wavelength, focal_length,
                          aperture_diameter, _unit_factor, _unit_label):
    """Display axis D4σ, PIB, Gaussian. Returns (axis_pib, axis_gaussian)."""
    st.header("光轴分析 — Axis Analysis")

    axis_pib = axis_gaussian = None
    if has_axis:
    ...
    return axis_pib, axis_gaussian



def _display_wavefront_analysis(has_both, axis_img, axis_features, pupil_img,
                            pupil_features, pupil_border, pupil_denoise,
                            wavelength, focal_length, axis_pixel,
                            pupil_pixel, max_zernike_order):
    """Display Strehl ratio and Zernike analysis. Returns zernike_result."""
    if not has_both:
        return dict(zernike_result=None, strehl=None)
    zernike_result = None
    strehl = None
    ...
    return dict(zernike_result=zernike_result, strehl=strehl)



def _display_results_summary(
        has_axis, has_pupil,
        axis_features, axis_pib, axis_gaussian,
        pupil_features, pupil_border, pupil_ellipse,
        uniformity_pupil, pupil_cross_section,
        pupil_type, pupil_gaussian_result,
        pupil_ftl_result, ftl_n_angles,
        bpp_result, M2, strehl, zernike_result,
        _unit_factor, _unit_label, uniformity_boundary_type):
    """Build and display the results summary table."""
    ...
    if has_pupil:
        ...
        if uniformity_pupil is not None:
            ...
            results["值"] += [
                ...
                f"{uniformity_pupil.get('rms_x', np.nan):.4f}",
                f"{uniformity_pupil.get('rms_y', np.nan):.4f}",
            ]
...
def main():
    st.set_page_config(page_title="AO光束质量分析", layout="wide")
    st.title("AO光束质量分析 — AO Beam Quality Analyzer")
    st.markdown("上传光轴(Axis)和/或光瞳(Pupil)图片，自动计算激光光束质量关键指标。")

    cfg = _render_sidebar()
    axis_pixel = cfg["axis_pixel"]
    pupil_pixel = cfg["pupil_pixel"]
    denoise_method = cfg["denoise_method"]
    manual_threshold = cfg["manual_threshold"]
    pupil_type = cfg["pupil_type"]
    uniformity_boundary_type = cfg["uniformity_boundary_type"]
    manual_center_x = cfg["manual_center_x"]
    manual_center_y = cfg["manual_center_y"]
    manual_radius = cfg["manual_radius"]
    ftl_n_angles = cfg["ftl_n_angles"]
    _unit_factor = cfg["_unit_factor"]
    _unit_label = cfg["_unit_label"]
    wavelength = cfg["wavelength"]
    focal_length = cfg["focal_length"]
    aperture_diameter = cfg["aperture_diameter"]
    max_zernike_order = cfg["max_zernike_order"]
    max_data_value = cfg.get("max_data_value", 255.0)

    col1, col2 = st.columns(2)
    with col1:
        st.subheader("光轴图片 (Axis)")
        axis_file = st.file_uploader("点击或拖拽上传光轴相机图片", type=["tiff", "tif", "png", "jpg", "jpeg"],
            help="支持 TIFF / PNG / JPG 格式。光轴为聚焦面图像（像方焦平面），用于计算 D4σ、PIB、高斯拟合直径。")
    with col2:
        st.subheader("光瞳图片 (Pupil)")
        pupil_file = st.file_uploader("点击或拖拽上传光瞳相机图片", type=["tiff", "tif", "png", "jpg", "jpeg"],
    ...
    if not has_axis and not has_pupil:
        ...
        upload_parts = []
        if has_axis: upload_parts.append("光轴")
        if has_pupil: upload_parts.append("光瞳")
        ...
        if st.button("🚀 开始计算", type="primary", width="stretch"):
            axis_img = axis_denoise = axis_black = None
            pupil_img = pupil_denoise = pupil_black = None
            axis_features = None
            pupil_features = pupil_border = pupil_ellipse = None
            uniformity_pupil = pupil_cross_section = None
            pupil_gaussian_result = pupil_ftl_result = None
            bpp_result = M2 = None

            scale_factor = max_data_value / 255.0
            ...
            if has_axis:
                ...
                axis_denoise = axis_denoise * scale_factor
            ...
            if has_pupil:
                ...
                pupil_denoise = pupil_denoise * scale_factor

            _display_3d_surface(has_axis, axis_denoise, has_pupil, pupil_denoise)

            result = _display_pupil_analysis(has_pupil, has_axis, axis_features, pupil_denoise, pupil_img,
                pupil_pixel, manual_center_x, manual_center_y, manual_radius,
                pupil_type, ftl_n_angles, _unit_factor, _unit_label,
                uniformity_boundary_type, wavelength, focal_length)
            pupil_features = result.get("pupil_features")
            pupil_border = result.get("pupil_border")
            pupil_ellipse = result.get("pupil_ellipse")
            uniformity_pupil = result.get("uniformity_pupil")
            pupil_cross_section = result.get("pupil_cross_section")
            pupil_gaussian_result = result.get("pupil_gaussian_result")
            pupil_ftl_result = result.get("pupil_ftl_result")
            bpp_result = result.get("bpp_result")
            M2 = result.get("M2")

            axis_pib, axis_gaussian = _display_axis_analysis(has_axis, axis_img, axis_denoise,
                axis_features, axis_pixel, wavelength, focal_length,
                aperture_diameter, _unit_factor, _unit_label)

            wf_result = _display_wavefront_analysis(has_axis and has_pupil,
                axis_img, axis_features, pupil_img, pupil_features,
                pupil_border, pupil_denoise, wavelength, focal_length,
                axis_pixel, pupil_pixel, max_zernike_order)
            zernike_result = wf_result.get("zernike_result")
            strehl = wf_result.get("strehl")
            _display_results_summary(has_axis, has_pupil,
                axis_features, axis_pib, axis_gaussian,
                pupil_features, pupil_border, pupil_ellipse,
                uniformity_pupil, pupil_cross_section,
                pupil_type, pupil_gaussian_result, pupil_ftl_result, ftl_n_angles,
                bpp_result, M2, strehl, zernike_result,
                _unit_factor, _unit_label, uniformity_boundary_type)
